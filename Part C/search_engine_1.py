...
class SearchEngine:
    ...
    def calculate_doc_weight(self):
        ...
        for word in self._indexer.inverted_idx:
            for doc_id, term_tf in self._indexer.inverted_idx[word]['posting_list']:
                ...
                self._indexer.docs_index[doc_id][0] += 0.8 * (term_tf / max_tf) * term_idf + 0.2 * (term_tf / doc_len) * term_idf
...
def main():
    ...
    n_res, res = se.search('coronavirus')
    print(n_res, res)
